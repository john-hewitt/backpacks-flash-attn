...
def build_bows_one_hot_vectors(bow_indices, tokenizer, device="cuda"):
    ...
    for single_bow in bow_indices:
        ...
        one_hot_bow = torch.zeros(num_words, 50264).to(device)
...
def full_text_generation(
    ...
    **kwargs
    ...
    for i in tqdm(range(num_samples), desc='[NUM SAMPLES]'):
...
def run_pplm_example(
    ...
    if 'flash' in pretrained_model:
      model_flash = SequenceLMModel.load_from_checkpoint('checkpoints/gpt2s-flash-fp16/last.ckpt').model
      config = model_flash.config
      config.output_hidden_states=True
      model = GPT2LMHeadModel(config)
      new_state_dict = demo_convert.remap_state_dict_flash(model_flash.state_dict(), config)
    ...
    for i, pert_gen_tok_text in enumerate(pert_gen_tok_texts):
        ...
        generated_texts.append((None, pert_gen_tok_text.detach().cpu(), None))
        #print(generated_texts)

    return [tokenizer.decode(x[1][0]) for x in generated_texts]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_path", default='out.txt', help="where to write the text")
    parser.add_argument("--negative_bow", default=False, help="whether to downweight bow terms")

    args = parser.parse_args()
    generations = run_pplm_example(**vars(args))
